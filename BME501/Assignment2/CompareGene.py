...
while (length > 1):
  ...
  Database = " NC" if (ncIndex < orIndex and ncIndex != -1)else " OR"
  GeneStrStartIndex = data.find("|") + 1
  GeneStrEndIndex= (data[GeneStrStartIndex:].find(" ["))+GeneStrStartIndex
  GeneStr = data[GeneStrStartIndex:GeneStrEndIndex] + Database
